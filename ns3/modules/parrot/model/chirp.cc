  ...
  if (remainingHops > 0 && postliminaryChecksPassed (incMultiHopChirp.GetOrig (), sender))
    {
      ...
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
           ...
           j != m_socketAddresses.end (); ++j)
        {
          ...
          // Differs from OMNeT++ implementation. Here, we create a new MultiHopChirp and discard the old one!
          ...
          // socket->SendTo (packet, 0, InetSocketAddress (destination, PARROT_PORT));
...
RoutingProtocol::SendMultiHopChirp ()
{
  ...
  //  filename << "trace_" << m_selfIpv4Address << ".csv";
